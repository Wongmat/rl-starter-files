...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    if variant in default_cfgs.keys():
        ...
        num_classes = default_num_classes = kwargs.pop("num_classes")
...
@register_model
def vit_small_patch16_160(pretrained=False, **kwargs):
    """ Adam's custom small ViT model. Depth=8, heads=8, mlp_ratio=3 """
    model_kwargs = dict(
        num_classes=1,
        img_size=160,
        patch_size=16,
    ...
    # NOTE: No pretrained model for now

    model = _create_vision_transformer(
        "vit_small_patch16_160", pretrained=False, **model_kwargs
    )
    return model
...
def vit_small_patch16_224(pretrained=False, **kwargs):
    ...
    Even so, this is a lot of parameters...may need to shrink embed_dim to something smaller than 768 or use smaller number of heads, smaller depth, etc.
    """
    model_kwargs = dict(
        patch_size=16,
        embed_dim=768,
...
if __name__ == "__main__":
    ...
    visual_bert_config = VisualBertConfig(
        ...
        intermediate_size=embedding_dim * mlp_ratio,
    )

    # model = VisualBertModel.from_pretrained("uclanlp/visualbert-vqa-coco-pre")
    model = VisualBertModel(visual_bert_config)
    # NOTE: this ViT assumes image size of 160. It must be 160x160 or else visual embeddings will be wrong
    vit = vit_small_patch16_160() # only use this to get image embeddings

    # NOTE: We can either use this tokenizer, our own tokenizer, or directly use text embeddings
    # from a2c repo
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    inputs = tokenizer("What is the man eating?", return_tensors="pt")

    img = cv2.imread("man_eating_sandwich.jpeg")
    img = cv2.resize(img, (160,160))
    img = img.transpose(2,0,1)
    img = torch.tensor(img).unsqueeze(0).float()
    visual_embeds, image_masks, patch_index, image_labels = vit.visual_embed(img, max_image_len=-1, mask_it=False)
